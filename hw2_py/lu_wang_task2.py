...
inputRDD = sc.textFile(sys.argv[3])
...
threshold = support / partitionNum
# print("\npartitionNum:\n", partitionNum)

rdd2 = data.groupByKey().mapValues(set).filter(lambda x: len(x[1]) > filter_threshold).map(lambda x: set(x[1]))
allbaskets = rdd2.collect()
# print(rdd2.collect())

# can_size_1 = rdd2.mapPartitions(fun_3).reduceByKey(lambda x, y: x+y).keys()
